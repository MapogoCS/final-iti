...
export class CartService {
  ...
  getApiCart() {
    ...
    if (user) {
      ...
      this.http
        .get(`https://furniture-eg.herokuapp.com/api/user/cart`, {
          params: queryParams,
        })
        .subscribe({
          next: (res: any) => {
  ...
  onStaitusChang(product: any, count: any) {
    ...
    this.http
      .post<{ status: number; message: string }>(
        `https://furniture-eg.herokuapp.com/api/update/cart`,
        postData
      )
      .subscribe({
        next: (res) => {
  ...
  postCart(postData: any, product: any) {
    this.http
      .post<{ status: number; message: string }>(
        `https://furniture-eg.herokuapp.com/api/cart`,
        postData
      )
      .subscribe({
        next: (res) => {
  ...
  deleteCartItem(product: any, queryParams: any) {
    ...
    this.http
      .delete(`https://furniture-eg.herokuapp.com/api/cart/${product_id}`, {
        params: queryParams,
      })
      .subscribe({
        next: (res) => {
  ...
  removeAllUserCart(user_id: any) {
    return this.http.delete(`https://furniture-eg.herokuapp.com/api/all-cart/${user_id}`);
  }
}
